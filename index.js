...
stream.on("tweet", async tweet => {
  ...
  if (
    tweet.user.id_str === "39538010" ||
    tweet.user.id_str === "286036879" ||
    tweet.user.id_str == "1068166867238494208"
  ) {
    //Get the statistics
    getStats().then(res => {
      let msg;
      if (res.difference <= 25000) {
        //SOUND THE ALARM!
        msg = `CALLING ALL 9 YEAR OLDS! SUBSCRIBE TO PEWDIEPIE NOW! PewDiePie [${humanize(
          res.pewdiepie
        )} subs] is currently ONLY ${humanize(
          res.difference
        )} subs away from TSeries [${humanize(res.tseries)} subs]!
				#SavePewDiePie #SubscribeToPewDiePie`;
      } else {
        //Okay we're a good number away
        msg = `Subscribe to PewDiePie! PewDiePie [${humanize(
          res.pewdiepie
        )} subs] is currently ${humanize(
          res.difference
        )} subs away from TSeries [${humanize(res.tseries)} subs]!
				#SavePewDiePie #SubscribeToPewDiePie`;
      }
